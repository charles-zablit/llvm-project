...
def skipUnlessFoundationEssentials(func):
    ...
    of it) ships in the Swift SDK on Darwin, Linux and Windows, so - unlike
    skipUnlessFoundation, which is Darwin-only - these tests run on all three.
    Use this for tests that only exercise the cross-platform Foundation subset
    and do not depend on Objective-C interop / full Apple Foundation.
    """
