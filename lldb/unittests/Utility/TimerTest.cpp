...
///
/// DumpCategoryTimes() sorts the categories by the time it measured for them,
/// so the order the lines appear in is not predictable: on a loaded machine a
/// category that is expected to be the faster one can end up being measured as
/// the slower one. Look the line up by category name rather than by position.
