...
class TestModuleAliasedPath(TestBase):
    ...
    def make_directory_alias(self, link, target):
        ...
        if sys.platform == "win32":
            # A junction, unlike a symbolic link, needs no extra privileges.
    ...
    def test_module_added_through_aliased_path(self):
        ...
        # Register the library the way the test harness does, but through the
        # alias, so that its path is not the one the process will report.
        self.assertTrue(
        ...
        # Let the loader find the library under its canonical path.
        env = []
        ...
        # The loaded image is the module that was already there, so the library
        # appears once and the breakpoint keeps its single, now resolved,
        # location rather than gaining a second, orphaned one.
        modules = [
            m
