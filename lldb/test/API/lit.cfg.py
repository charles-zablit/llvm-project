...
# FoundationEssentials, ...). Prefer an explicit runtime-bin path if the
# build system supplies one (e.g. the bootstrap layout where the runtime
# lives beside the toolchain image rather than inside the SDK), and fall
# back to <sysroot>/usr/bin for builds that still co-locate the runtime
# with the SDK.
