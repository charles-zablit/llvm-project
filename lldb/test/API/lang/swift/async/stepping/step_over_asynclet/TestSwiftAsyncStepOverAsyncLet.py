...
class TestCase(lldbtest.TestBase):
    ...
    @skipEmbeddedSwift
    @swiftTest
    # A blocking await resumes its continuation on a different cooperative-pool
    # thread than the one the step began on, so this needs the Swift tasks
    # plugin to keep presenting the task as the same thread across that hop.
    # Left gated on Linux only because that has not been verified there.
    @skipIf(oslist=["linux"])
