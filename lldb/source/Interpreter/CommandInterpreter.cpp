...
namespace {
CommandInterpreter::StandardCommandsLoader g_standard_commands_loader = nullptr;
}

void CommandInterpreter::SetStandardCommandsLoader(
    StandardCommandsLoader loader) {
  g_standard_commands_loader = loader;
}

void CommandInterpreter::LoadCommandDictionary() {
  LLDB_SCOPED_TIMER();

  // The concrete CommandObject* implementations live in lldbCommands; tools
  // that link the framework but not lldbCommands (notably lldb-server) leave
  // this hook unset and run with an empty command dictionary. liblldb's
  // SystemInitializerFull installs the hook so that an interactive lldb gets
  // its full command set.
  if (g_standard_commands_loader)
    g_standard_commands_loader(*this);
}
