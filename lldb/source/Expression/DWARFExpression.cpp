...
static llvm::Expected<Value> SwiftAsyncEvaluate_DW_OP_entry_value(
  ...
  //     is already dereferenced -> the DWARF DW_OP_deref is redundant, consume it;
  ...
  if (SwiftLanguageRuntime::IsSwiftAsyncAwaitResumePartialFunctionSymbol(
    ...
    if (!cfa_is_indirect_context && target) {
      if (target->GetArchitecture().GetTriple().isArm64e()) {
        ...
        cfa_is_indirect_context =
            func_load != LLDB_INVALID_ADDRESS &&
            pc_load != LLDB_INVALID_ADDRESS && pc_load >= func_load &&
