...
#if defined(_WIN32)
    // On Windows we use anonymous pipes rather than a ConPTY for stdio
    // forwarding, so no terminal emulation is performed on the server side.
    // The platform-specific NativeProcessProtocol (NativeProcessWindows)
    // takes ownership of the PseudoConsole and forwards child output
    // through the NewProcessOutput delegate hook; see
    // `GDBRemoteCommunicationServerLLGS::NewProcessOutput`.
...
GDBRemoteCommunicationServerLLGS::PrepareStopReplyPacketForThread(
  ...
  // Advertise any library load/unload events that happened since the last
  // stop so the client knows to re-read the module list via
  // `qXfer:libraries:read`. Processes that do not track library events just
  // return false from HasPendingLibraryEvents (the default implementation),
  // so this is a no-op on platforms without library-list support. The flag
  // stays set until the process resumes, so clients can re-query `?` and
...
GDBRemoteCommunicationServerLLGS::ReadXferObject(llvm::StringRef object,
  ...
  if (object == "libraries") {
    ...
    for (auto const &library : *library_list) {
      response.Printf("<library name=\"%s\">",
                      XMLEncodeAttributeValue(library.name.c_str()).c_str());
      response.Printf("<section address=\"0x%" PRIx64 "\"/>", library.base_addr);
