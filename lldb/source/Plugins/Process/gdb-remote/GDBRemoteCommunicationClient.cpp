...
int GDBRemoteCommunicationClient::SetSTDIOWindowSize(uint16_t cols,
  ...
  if (cols == 0 || rows == 0)
    return -1;
  StreamString packet;
  packet.Printf("QSetSTDIOWindowSize:cols=%u;rows=%u", static_cast<unsigned>(cols),
  ...
  if (response.IsOKResponse())
    return 0;
  // Older / non-Windows servers will return "" (unsupported). Treat that as a
  // soft success — we just leave the inferior with the server's default size.
  if (response.IsUnsupportedResponse())
    return 0;
  uint8_t error = response.GetError();
