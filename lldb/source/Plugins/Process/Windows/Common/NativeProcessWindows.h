...
private:
  ...
  /// launcher installs the ConPTY into the child via STARTUPINFOEX, and we
  /// take ownership of the parent-side handles here via TakePTY() so the
  /// reader thread can keep pumping after launch_info is cleared.
  std::shared_ptr<PseudoConsole> m_pty;

  /// ThreadedCommunication that wraps a ConnectionConPTY around the PTY's
  /// parent-side STDOUT HANDLE. Started in the launch ctor and stopped in
  /// the destructor / OnExitProcess.
  ThreadedCommunication m_stdio_communication;

  /// Bridge between m_stdio_communication's read thread and
  /// NativeDelegate::NewProcessOutput. Static so it can be passed to
  /// SetReadThreadBytesReceivedCallback.
  static void STDIOReadThreadBytesReceived(void *baton, const void *src,
                                           size_t src_len);

  /// Wire up m_stdio_communication on m_pty's STDOUT HANDLE. No-op if the
  /// PTY is null or not connected.
  void StartStdioForwarding();

  /// Tear down the read thread and disconnect m_stdio_communication. Safe
