...
private:
  ...
  // The current exception waiting to be handled.  Written by the debug loop
  // when it reports an exception and cleared by ContinueAsyncException(), which
  // runs on the thread driving the debugger; m_active_exception_mutex
  // serializes the two.
