...
bool lldb_private::formatters::swift::Date_SummaryProvider(
  ...
  if (!tm_date)
    return false;
  // The broken-down time from gmtime() is always UTC, so the zone suffix is
  // always UTC. Don't use strftime("%Z"): on Windows it ignores the gmtime
  // result and prints the process's local zone name (e.g. "GMT Standard
  // Time") instead of "UTC".
