...
LLDBMemoryReader::getSymbolAddress(const std::string &name) {
  ...
  // Remove undefined symbols, and prefer a symbol whose load address resolves.
  // On Windows a secondary DLL can appear in the target as a stale duplicate
  // module that contributes matches with no load address; picking a resolvable
  // candidate yields the correct symbol (e.g. an opaque type descriptor)
  // address. Fall back to the defined set (then the original list) so the
  // no-process / single-symbol cases are unaffected.
  {
