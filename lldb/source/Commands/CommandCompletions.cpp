...
static bool DispatchCommonCompletionCallbacks(CommandInterpreter &interpreter,
...
// Hook entry point installed by SystemInitializerFull (liblldb) and
// SystemInitializerTest (lldb-test). lldb-server intentionally leaves this
// unset so InvokeCommonCompletionCallbacks degrades to a no-op and the binary
// can drop the entire CommandObject* graph from its link line.
namespace lldb_private {
void RegisterCommonCompletionDispatcher() {
  CommandCompletions::SetCommonCompletionDispatcher(
      &DispatchCommonCompletionCallbacks);
}
} // namespace lldb_private
...
void CommandCompletions::DiskFiles(CommandInterpreter &interpreter,
  ...
  path_completion::Complete(request.GetCursorArgumentPrefix(),
                            /*only_directories=*/false, request, resolver);
}

void CommandCompletions::DiskFiles(const llvm::Twine &partial_file_name,
                                   StringList &matches,
                                   TildeExpressionResolver &Resolver) {
  path_completion::CompleteFiles(partial_file_name, matches, Resolver);
}
...
void CommandCompletions::DiskDirectories(CommandInterpreter &interpreter,
  ...
  StandardTildeExpressionResolver resolver;
  path_completion::Complete(request.GetCursorArgumentPrefix(),
                            /*only_directories=*/true, request, resolver);
}

void CommandCompletions::DiskDirectories(const llvm::Twine &partial_file_name,
                                         StringList &matches,
                                         TildeExpressionResolver &Resolver) {
  path_completion::CompleteDirectories(partial_file_name, matches, Resolver);
}
