...
llvm::Error PseudoConsole::OpenPseudoConsole(uint16_t req_cols,
  ...
  if (req_cols != 0 && req_rows != 0) {
    // Client-supplied dimensions (gdb-remote QSetSTDIOWindowSize). Trust them
    // verbatim — the caller has already mapped a "no real terminal" scenario
    // to a sensible wide value to avoid wrapping inferior output.
