...
// sockaddr_un and the socket calls come from different headers on each
// platform. This is the only place the cross-platform domain-socket code needs
// a platform conditional; everything below operates on the standard socket API.
#ifdef _WIN32
...
static bool SetSockAddr(llvm::StringRef name, const size_t name_offset,
  ...
  // is not available on Windows, and for a regular (non-abstract) domain
  // socket SUN_LEN(saddr_un) is by definition
  //   offsetof(sockaddr_un, sun_path) + strlen(sun_path)
  // which, since sun_path was memset to zero and the name copied at offset 0,
  // equals offsetof(...) + name.size(). For abstract sockets (name_offset==1,
  // Linux only) the leading NUL means the length must be computed manually.
  // The single expression below is correct for both cases on every platform.
  saddr_un_len =
